    ...
    <div class="navbar-inner">
        ...
        <div class="nav-user">
            <span class="user-pill">
                <span class="user-avatar"><?= strtoupper(substr($user['name'] ?? 'A', 0, 1)) ?></span>
...
<main class="main-content">
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Register, edit, search and remove teacher accounts</p>
        </div>
    </div>

    <?php if (isset($_GET['msg'])): ?>
        <?php $messages = ['added' => 'Teacher added successfully.',
    ...
    <div class="card form-card">
        <h3 class="card-title">
            <?= $isEdit ? '&#9998; Edit Teacher (#' . intval($editing['id']) . ')' : '+ Add New Teacher' ?>
        </h3>
        <form method="POST"
    ...
    <div class="card">
        <div class="card-toolbar">
            ...
            <span class="badge" id="resultCount"><?= count($teachers) ?> total</span>
        </div>
            ...
            <table class="data-table">
                ...
                <tbody id="tableBody">
                    <?php if (empty($teachers)): ?>
                        <tr><td colspan="5" class="empty">No teachers yet.</td></tr>
                    <?php else: ?>
                        <?php foreach ($teachers as $i => $teach): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= htmlspecialchars($teach['name']) ?></td>
                                <td><?= htmlspecialchars($teach['contact']) ?></td>
                                <td><?= htmlspecialchars($teach['username']) ?></td>
                                <td class="text-right">
                                    <a class="btn-sm btn-edit"
                                       href="index.php?page=admin&action=edit&id=<?= $teach['id'] ?>">Edit</a>
                                    <a class="btn-sm btn-delete"
                                       href="index.php?page=admin&action=delete&id=<?= $teach['id'] ?>"
...
(function () {
    ...
    input.addEventListener('input', function () {
        ...
        timer = setTimeout(function () {
            fetch('index.php?page=ajax&type=teacher&q=' + encodeURIComponent(input.value.trim()),
