...
/* ------------- AJAX search endpoint -------------
   ...
   ?page=ajax&type=teacher&q=...   (admin only)
   ?page=ajax&type=course&q=...    (teacher only)
*/
if ($page === 'ajax') {
    ...
    if ($type === 'teacher' && $_SESSION['user']['role'] === 'admin') {
        echo json_encode($q === '' ? getTeachers($conn) : searchTeachers($conn, $q));
    } elseif ($type === 'course' && $_SESSION['user']['role'] === 'teacher') {
...
if ($page === 'teacher' && $_SESSION['user']['role'] !== 'teacher') { header('Location: index.php?page=login'); exit; }
...
switch ($page) {
    ...
    case 'teacher':  teacherCtrl($conn);  break;
    default:
        header('Location: index.php?page=login');
        exit;
}
